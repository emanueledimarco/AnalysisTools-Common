...
{
  cout << "generatedFile = " << generatedFile << endl;
...
double LumiReWeighting::ITweight( int npv ){
  int bin = weights_->GetXaxis()->FindBin( npv );
  return weights_->GetBinContent( bin );
}

double LumiReWeighting::ITweight3BX( float ave_int ){
  int bin = weights_->GetXaxis()->FindBin( ave_int );
  return weights_->GetBinContent( bin );
}

double LumiReWeighting::weightOOT( int npv_in_time, int npv_m50nsBX ){
